...
export abstract class MessageGenerator {
  ...
  static getEmailVerificationCodeMessage(token: string): string {
	...
	static getForgetPasswordMessage(user: ExtendedUser, token: string): string {
		const {Frontend_Verity_Password_Token_Route} = process.env;
    return`<!DOCTYPE html>
		<html lang="en">
		<head>
				<meta charset="UTF-8">
		...
		<body>
			<div>
				...
				<p style="color: black">This is your reset password link</p
				<a href="${Frontend_Verity_Password_Token_Route}?email=${user.email}&token=${token}">Reset Password</a>
				<p>If you did not register for our service, you can safely ignore this email.</p>
				<p>
					<span>Best regards,</span>
					<br/>
